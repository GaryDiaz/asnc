...
class LlamadoConcursoRest extends RestController {
	...
	public function buscar_por_texto_get($textoABuscar, $propio) {
		$rif = $this->extraerRifPropio($propio);
		try {
			$this->load->model('dao/LlamadoConcursoDAO');
	...
	public function buscar_por_objeto_contratacion_get($idObjetoContratacion, $propio) {
		$rif = $this->extraerRifPropio($propio);
		try {
			$this->load->model('dao/LlamadoConcursoDAO');
			$res = $this->LlamadoConcursoDAO->buscarPorObjetoContratacion($idObjetoContratacion, $rif);
			if ($res) {
				$data = new Mensaje("Lista de Llamado a Concurso por Objeto de Contratación");
				$data->setDatos($res, "Lista");
				$this->response($data, self::HTTP_OK);
			} else {
				$this->response(new Mensaje("No se encontraron llamados a concurso por el objeto de contratación indicado"), self::HTTP_BAD_REQUEST);
			}
		} catch (Exception $exc) {
			$this->response(new Mensaje($exc->getMessage()), self::HTTP_BAD_REQUEST);
		}
	...
	public function llamado_concurso_put($rif, $numero_proceso) {
		...
		try {
			...
			if ($rs) {
				...
				$this->response(new Mensaje("No se pudo editar el llamado a concurso"), self::HTTP_BAD_REQUEST);
			}
		} catch (Exception $exc) {
			$this->response(new Mensaje($exc->getMessage()), RestController::HTTP_BAD_REQUEST);
		}
	...
	public function llamado_concurso_delete($rif, $numero_proceso) {
		...
		try {
			...
			if ($rs) {
				...
				$this->response(new Mensaje("No se pudo eliminar el llamado a concurso"), self::HTTP_BAD_REQUEST);
			}
		} catch (Exception $exc) {
			$this->response(new Mensaje($exc->getMessage()), RestController::HTTP_BAD_REQUEST);
		}
	...
	*/
	...
	private function extraerRifPropio(int $propio) {
		$rif = null;
		if ($propio) {
			$this->sesionIniciada();
			$rif = $this->session->userdata['rif_organoente'];
